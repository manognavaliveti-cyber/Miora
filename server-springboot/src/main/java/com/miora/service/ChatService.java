...
public class ChatService {
    ...
    public Message sendMessage(String matchId, String senderId, String text, String type, Map<String, Object> metadata) {
        ...
        if (match == null) {
            throw new SecurityException("User not a participant of this match");
        }


        // Verify the authenticated user is a participant of this match (owner "user_me" or the matched profile)
        if (!senderId.equals(match.getProfileId()) && !"user_me".equals(senderId)) {
            throw new SecurityException("User not a participant of this match");
        }
...
// Mark messages as read with security checks
    public void markMessagesAsRead(String matchId, String userId) {
        // Verify match exists
        Match match = firestoreService.getMatches().stream()
                .filter(m -> matchId.equals(m.getId()))
                .findFirst()
                .orElseThrow(() -> new IllegalArgumentException("Match not found"));
        
        // Verify the authenticated user is a participant of this match (owner "user_me" or the matched profile)
        if (!userId.equals(match.getProfileId()) && !"user_me".equals(userId)) {
            throw new SecurityException("User not a participant of this match");
        }

        // Delegate to FirestoreService to update read status and timestamp
        firestoreService.markMessagesAsRead(matchId, userId);
    }
}

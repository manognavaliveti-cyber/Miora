...
public class UserService {
    ...
    public User updateUser(String userId, User updateData) {
        ...
        if (updateData.getFcmToken() != null && !updateData.getFcmToken().trim().isEmpty()) {
            existing.setFcmToken(updateData.getFcmToken().trim());
        }
    ...
    /**
     * Secure Permanent Account Deletion Workflow (Requirements 5 & 6)
     * 1. Mark status DELETION_PENDING
     * 2. Anonymize/clean user data in Firestore
     * 3. Delete Firebase Auth user using Firebase Admin SDK
     * 4. Finalize DELETED status
     */
    public boolean deleteAccount(String userId) {
        ...
        user.setStatus("DELETION_PENDING");
        user.setUpdatedAt(Instant.now().toString());
        firestoreService.saveUser(user);

        // STEP 2: Anonymize and clean up user data in Firestore (likes, passes, matches, follows, stories, notes)
        firestoreService.purgeUserData(userId);
        ...
        // STEP 3: Delete Firebase Auth account using Firebase Admin SDK
        if (firebaseAuth != null) {
            try {
                firebaseAuth.deleteUser(userId);
                log.info("Deleted Firebase Auth user UID: {}", userId);
            } catch (Exception e) {
                log.warn("Notice: Could not delete Firebase Auth user {} (may be dev mode or already deleted): {}", userId, e.getMessage());
            }
        }

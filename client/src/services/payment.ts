...
export class RazorpayPaymentService {
   ...
   * 4. Updates wallet balance / Premium status ONLY after server verification succeeds
   */
  async initiateCheckout(options: RazorpayCheckoutOptions): Promise<void> {
    try {
      const targetId = options.productId || options.packageId || 'pack_150';
      // 1. Request authentic order from Spring Boot backend
      const orderData = await apiService.createPaymentOrder(targetId);
      ...
      const isPremiumProduct = (orderData.productType || '').includes('SUBSCRIPTION') || targetId.includes('gold') || targetId.includes('vip') || targetId.includes('premium');
      const desc = isPremiumProduct
        ? 'MIORA Premium Subscription 20% OFF (TEST)'
        : `${orderData.productName || 'MIORA ₹ Wallet Credit'} (TEST)`;
        ...
        handler: async (response: {
          ...
          try {
            ...
            const verifyRes = await apiService.verifyPayment({
              ...
              productId: orderData.productId || orderData.packageId || targetId,
              packageId: orderData.packageId || targetId
            });

            if (verifyRes && verifyRes.success) {
              options.onSuccess({
                newWalletBalance: verifyRes.newWalletBalance ?? verifyRes.newBalance ?? 0,
                creditAddedInr: verifyRes.creditAddedInr ?? verifyRes.coinsAdded ?? 0,
                isPremium: verifyRes.isPremium,
                subscriptionTier: verifyRes.subscriptionTier,

    ...
    <>
      ...
      >
        ...
        {isVideo ? (
          ...
          >
            {remoteCallStream ? (
              ...
              <img src={partnerPhoto} alt={partner.name} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.55) blur(4px)' }} />
            )}
            {/* Gradient Overlays */}
            <div
              style={{
        ...
        >
          ...
          {/* Video Call User PiP */}
          {isVideo && (
            <div style={{ position: 'absolute', top: '20px', right: '24px', width: '120px', height: '170px', borderRadius: '20px', overflow: 'hidden', boxShadow: '0 12px 32px rgba(0,0,0,0.6)', border: '2px solid rgba(255,255,255,0.3)', background: '#1F161A', zIndex: 20 }}>
              <video ref={localVideoRef} autoPlay muted playsInline style={{ width: '100%', height: '100%', objectFit: 'cover', transform: activeCall.isFrontCamera ? 'scaleX(-1)' : 'none', display: activeCall.isCameraOff ? 'none' : 'block' }} />
              {activeCall.isCameraOff && <img src={userPhoto} alt={currentUser.name} style={{ width: '100%', height: '100%', objectFit: 'cover', filter: 'brightness(0.45)' }} />}
              <span style={{ position: 'absolute', bottom: '8px', left: '8px', background: 'rgba(0,0,0,0.6)', color: '#FFFFFF', fontSize: '0.65rem', fontWeight: 700, padding: '2px 6px', borderRadius: '6px' }}>You</span>
            </div>
          )}
        </main>

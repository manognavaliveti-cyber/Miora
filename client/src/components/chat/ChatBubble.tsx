...
import { Play, Pause, Zap, Sparkles, Volume2, Mic, Trash2, Check, CheckCheck } from 'lucide-react';
...
const ChatBubbleContent: React.FC<ChatBubbleProps> = ({ message, isMe, avatarUrl }) => {
    ...
    return (
      ...
      >
        ...
        >
          ...
          {message.read && (
            <span style={{ fontSize: '0.72rem', color: '#10B981', padding: '0 4px', fontWeight: 500, marginLeft: '6px' }}>
              Seen
            </span>
          )}
        </div>
...
export const ChatBubble: React.FC<ChatBubbleProps> = (props) => {
  const { message, isMe, onDeleteMessage } = props;
  const canDelete = isMe && !!onDeleteMessage;

  return (
    <div className="chat-message-wrapper" style={{ position: 'relative', width: '100%' }}>
      <ChatBubbleContent {...props} />
      {isMe && (
        <span
          className="chat-message-read-status"
          aria-label={message.read ? 'Seen' : 'Sent'}
          title={message.read ? 'Seen' : 'Sent'}
        >
      ...
      {canDelete && (
        <button
          ...
          onClick={(event) => {
            ...
            if (window.confirm('Delete this message?')) onDeleteMessage?.(message);
          }}
        >
          <Trash2 size={13} />
        </button>
